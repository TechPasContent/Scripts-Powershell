...
$NewVolumeName = "Data"
$bInstallDHCP = (Get-Content $PSScriptRoot\general.conf)[16].Split()[2]
if ($bInstallDHCP -ieq "false") { $bInstallDHCP = $false} else { $bInstallDHCP = $true }
$bInstallShare = (Get-Content $PSScriptRoot\general.conf)[16].Split()[2]
if ($bInstallShare -ieq "false") { $bInstallShare = $false} else { $bInstallShare = $true }
